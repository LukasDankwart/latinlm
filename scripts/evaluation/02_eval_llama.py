...
from src.tests.llamar_metrics import analyze_sentence_by_llm, initialize_deepseek_api, distinct_n_repetition, type_token_ratio, compute_conventional_metric_summary, compute_llm_judgement_summary
import argparse
import os
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(f"--skip-summary", action="store_true")
...
def main():
    ...
    if perform_inference:
        ...
        perform_autoregressive_completion(
    ...
    # 7. Step: ───── Storing complete evaluation results to csv ─────
    ...
    if not args.skip_summary:
        ...
        print(f"-- [INFO] Computing conventional metrics for sources/bins...")
        source_metrics, bin_metrics = compute_conventional_metric_summary(final_results)
        source_metric_path = os.path.join(output_dir, "eval_conventional_source_metrics.csv")
        source_df = pd.DataFrame(source_metrics)
        source_df.to_csv(source_metric_path)
        print(f"-- [INFO] Stored source metrics to '{source_metric_path}'")

        bin_metrics_path = os.path.join(output_dir, "eval_conventional_bin_metrics.csv")
        bin_df = pd.DataFrame(bin_metrics)
        bin_df.to_csv(bin_metrics_path)
        print(f"-- [INFO] Stored bin metrics to '{bin_metrics_path}'")
        ...
        judge_source_metrics_path = os.path.join(output_dir, "eval_judgement_source_metrics.csv")
        judge_source_df = pd.DataFrame(judge_source_metrics)
        judge_source_df.to_csv(judge_source_metrics_path)
        print(f"-- [INFO] Stored source LLM-judgement results to '{judge_source_metrics_path}'")
